...
# Load the model
model = pickle.load(open('model(16).pkl', 'rb'))

# Define the local path to the dataset
DATASET_PATH = 'df2.csv'
...
def predict_and_plot():
    try:
        ...
        required_columns = ['a1', 'a2', 'a3', 'a4', 'load']
        ...
        sns.distplot(df['failure'], label='Gearbox Failure', color='yellow')  # Yellow for actual
        sns.distplot(df['Prediction'], label='Prediction', color='red')  # Red for predicted
        plt.title('Distribution of Gearbox Failure and Prediction', color='white')
    ...
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                point = Point("Gear Box Fault") \
                    .field("Actual_Failure", float(row['failure'])) \
                    .field("Predicted_Failure", float(row['Prediction'])) \
